...
is Nathan's jachammer account and because **T2 is the only track with a free first hour** —
everything else in it waits on #1.

**The producer/consumer seams are intentional.** `Recall` (T2) produces the `Concern` list `Prepare`
(T3) consumes; `Remember` (T1) writes the graph `Recall` reads. Every seam is a pinned shape in
`CONTRACTS.md` with a stub on the producing side, which is what lets all three move at once.
...
**Nathan runs #30 — issue zero.** Five platform assumptions become facts: node/edge persistence
across a preview restart, `root spawn` from inside `cl { }`, `by llm()` returning a typed obj, a
project-scoped env var readable at runtime, and a sandbox deploy serving a page. It also records
**the graph reset path**, which is what unblocks #1.
...
```
<track>: <what changed>            e.g. T2: Vigil emits silence Concern on observation gap
...
This is what keeps three tracks moving on one graph.
